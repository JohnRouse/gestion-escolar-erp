...
| Citas | sí | sí | sí | completo | CASI COMPLETO | P0 | Agenda individual y reuniones de sección, estados/acuerdos, audiencia derivada y contrato del portal en pruebas; aplicar la segunda migración de forma controlada y aceptar el flujo real cuando exista autenticación externa segura. **A:** revisión dirigida en curso. **D:** migración local pendiente, autenticación externa y bandeja de Notificaciones. |
...
**Recuento:** 65 capacidades: COMPLETO: 1, CASI COMPLETO: 38, PARCIAL: 16, ESQUELETO: 7, NO INICIADO: 3. No se asignan porcentajes de avance. Staff y Citas son las reclasificaciones posteriores al inventario: Citas no es COMPLETO porque el acceso externo aún impide aceptar el recorrido del apoderado. No se reauditaron las demás capacidades ni se certifica la campaña E2E integral.
...
4. **Completar módulos escolares pendientes:** aceptar agenda institucional de Citas y su migración de reuniones, Enfermería, bandeja de Notificaciones y gestión de eventos. Integrar avisos y acceso por institución; conservar trazabilidad de salud y acuerdos según su sensibilidad.
...
**Primer incremento del bloque 1 — Staff COMPLETO:** listado, búsqueda, alta/edición institucional, disponibilidad de citas y creación/asociación de credenciales con membresías mínimas, reutilizando Persona/Usuario/Staff. La aceptación funcional dirigida inicial aprobó 20/20 pruebas backend en MySQL aislado, builds y ESLint dirigidos, y Playwright de escritorio/móvil, permisos y accesibilidad funcional. El incremento UX posterior completó la puerta visual V1 con lista tabular en escritorio, fichas estructuradas en pantallas angostas, filtros compactos y formulario agrupado; se revisaron 1440×900, 1280×800, 768×900, 390×844 y zoom equivalente al 200 %, sin desbordamiento ni pérdida de acciones. El cierre de separación funcional añade `es_miembro_staff`: ser Docente o Tutor no basta para aparecer en Staff; una Persona con ambos dominios conserva una sola identidad y cada módulo muestra su función. El último incremento permite a Admin/Director corregir la Persona canónica dentro de autoridad completa, incluido DNI único, y administrar username, rol, contraseña y estado de la cuenta existente con motivo y jerarquía backend. Docentes identifica al tutor mediante `tutorias_resumen` y conserva su sección. Tutoría sigue operando con `es_tutor`/`id_seccion` como compatibilidad técnica hasta su migración académica. `permite_citas` no es activación laboral: Staff no tiene estado laboral. Ciclo laboral, traslados, clasificación asistida de históricos, migración de la relación académica de Tutoría, consola general de Usuarios/Roles y auditoría persistente quedan fuera; Staff solo administra la cuenta y membresía ligadas a su contexto autorizado. Esto no declara completo Usuarios/Roles ni el bloque 1.

**Segundo incremento del bloque 1 — Citas CASI COMPLETO:** agenda interna,
creación individual en nombre de la familia, reuniones de sección, responsables
Docente/Tutor/Staff, búsqueda incremental, audiencia derivada, estados,
reprogramación, acuerdos, historial y autorización contextual están conectados y
