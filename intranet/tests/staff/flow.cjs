...
(async () => {
  ...
  const browser = await chromium.launch({
    ...
    env: {
      ...process.env,
      XDG_CONFIG_HOME: '/tmp/staff-v1-chrome-config',
      XDG_CACHE_HOME: '/tmp/staff-v1-chrome-cache',
    },
  });
  try {
    for (const viewport of [
      ...
      const dialog = page.getByRole('dialog', {
        name: 'Nuevo miembro de Staff',
      });
      await dialog.waitFor();
      assert.equal(
        await dialog.getByLabel('DNI *', { exact: true }).evaluate((el) => el === document.activeElement),
        true,
      );
      ...
      assert.equal(
        await page
          .getByRole('button', { name: 'Nuevo miembro', exact: true })
      ...
      await dialog.getByLabel('Departamento', { exact: true }).selectOption('Lima');
      await dialog.getByLabel('Provincia', { exact: true }).selectOption('Lima');
      await dialog.getByLabel('Distrito', { exact: true }).selectOption('Lima');
      await dialog.getByLabel('Dirección', { exact: true }).fill('Av. Pruebas 123');
      ...
      await dialog.getByLabel('Dar acceso al ERP').check();
      ...
      await dialog.getByLabel('Motivo para otorgar acceso *').fill('Necesita operar procesos administrativos');
      await page.screenshot({
        path: `/tmp/staff-v1-form-${viewport.width}.png`,
        fullPage: true,
      });
      ...
      const edit = page.getByRole('button', {
        name: 'Editar Prueba Navegador Staff',
        exact: true,
      });
      await edit.waitFor();
      await edit.click();
      const editor = page.getByRole('dialog', {
        name: 'Editar miembro de Staff',
      });
      await editor.waitFor();
      assert.equal(await editor.getByLabel('Dirección', { exact: true }).inputValue(), 'Av. Pruebas 123');
      assert.equal(await editor.getByLabel('Departamento', { exact: true }).inputValue(), 'Lima');
      assert.equal(await editor.getByLabel('Provincia', { exact: true }).inputValue(), 'Lima');
      assert.equal(await editor.getByLabel('Distrito', { exact: true }).inputValue(), 'Lima');
      await editor.getByText(`staff.browser.${dni}`, { exact: true }).waitFor();
      await editor.getByText('Secretaria', { exact: true }).waitFor();
      await editor.getByText('Acceso activo', { exact: true }).waitFor();
      assert.equal(await editor.getByLabel('Dar acceso al ERP').count(), 0);
      assert.equal(await editor.getByLabel('Motivo para otorgar acceso *').count(), 0);
      assert.equal(await editor.getByText(/Tutor asignado|Tutoría y sección asignada/).count(), 0);
      ...
      await page.screenshot({
        path: `/tmp/staff-v1-list-${viewport.width}.png`,
        fullPage: true,
      });
      ...
      console.log(
        `PASS ${viewport.width}x${viewport.height}: Admin login, list, search, validation, isolated create+credentials, edit, appointments off/on, keyboard/focus, 200% zoom, reduced motion, no console errors`,
      );
      await context.close();
    }
    ...
    const login = await context.request.post(`${url}/api/auth/login`, {
      data: { username: 'staff.profesor', password },
    });
    ...
    const denied = await context.request.post(`${url}/api/staff?tenant_id=1&scope=all`, {
