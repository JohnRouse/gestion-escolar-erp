...
interface UltimaMatricula {
  ...
  registrado_por?: {
    username: string; rol?: { nombre_rol: string };
    persona?: { nombres: string; apellido_paterno: string; apellido_materno?: string };
  } | null;
  estudiante: {
    codigo_estudiante?: string; codigos_colegio?: CodigoColegio[];
    persona: { dni?: string; nombres: string; apellido_paterno: string };
  };
...
const emptyAlumno: PersonaForm = {
  ...
  direccion: '', pais: 'Perú', departamento: '', provincia: '', distrito: '',
};

const emptyApoderado: PersonaForm = {
  dni: '', nombres: '', apellido_paterno: '', apellido_materno: '',
  telefono: '', correo: '', direccion: '', pais: 'Perú',
  departamento: '', provincia: '', distrito: '', ocupacion: '',
};
...
const getEstadoCls = (estado?: string) =>
  ESTADO_BADGE[estado || ''] || 'bg-slate-100 text-slate-500 ring-1 ring-slate-200';
...
const calcularEdadDetallada = (fechaNacimiento?: string | null, fechaCorte?: Date) => {
  ...
  if (Number.isNaN(nacimiento.getTime()) || Number.isNaN(corte.getTime()) || nacimiento > corte) return null;
  ...
  if (dias < 0) { meses -= 1; dias += new Date(corte.getFullYear(), corte.getMonth(), 0).getDate(); }
  if (meses < 0) { anios -= 1; meses += 12; }
  const partes = [`${anios} ${anios === 1 ? 'año' : 'años'}`];
  if (meses > 0) partes.push(`${meses} ${meses === 1 ? 'mes' : 'meses'}`);
  return { anios, meses, dias, totalMeses: anios * 12 + meses, texto: partes.length === 1 ? partes[0] : `${partes.slice(0,-1).join(', ')} y ${partes[partes.length-1]}` };
};
...
const formatFechaHora = (value: string) =>
  new Date(value).toLocaleString('es-PE', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });

const formatMoney = (value: number | string | null | undefined) =>
  `S/ ${Number(value || 0).toFixed(2)}`;

const getCodigoInstitucional = (matricula: { id_colegio?: number | null; estudiante?: { codigo_estudiante?: string | null; codigos_colegio?: CodigoColegio[] } }) => {
  const codigoColegio = matricula.estudiante?.codigos_colegio?.find((item) => item.id_colegio === matricula.id_colegio);
  return codigoColegio?.codigo || matricula.estudiante?.codigo_estudiante || 'Sin código';
};

const getCodigoMatricula = (matricula: { id_matricula: number; codigo_matricula?: string | null }) =>
...
const getCodigoDetalleMatricula = (detalle: any) => {
  ...
  const codigoColegio = detalle.estudiante?.codigos_colegio?.find((item: CodigoColegio) => item.id_colegio === detalle.id_colegio);
...
const validarFechaNacimientoFrontend = (fecha?: string) => {
  ...
  if (nacimiento < minima) return 'La fecha de nacimiento parece demasiado antigua. Revisa el dato.';
  return null;
};
...
export default function MatriculaPage() {
  ...
  const colegioDestinoNombre = useMemo(() => {
    if (activeScope.tipo === 'colegio') return activeColegio?.nombre_corto || activeColegio?.nombre || 'Colegio activo';
  ...
  const getEstadoOperativoAnioFrontend = (anio?: Anio | null) => {
    const estado = String(anio?.estado || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
  ...
  const esAnioDisponibleParaRegistro = (anio?: Anio | null) => {
    ...
    if (estadoOperativo === 'Cerrado' || estadoOperativo === 'Archivado') return false;
    if (anio.fecha_fin) {
  ...
  const tiposIngresoPermitidos = useMemo(() => {
    if (!anioSeleccionado || !esAnioDisponibleParaRegistro(anioSeleccionado)) return [];
    const estadoOperativo = getEstadoOperativoAnioFrontend(anioSeleccionado);
    if (estadoOperativo === 'Planificación') return ['Reserva','Renovación','Renovación con cambio de sede'];
    const anioCorte = getAnioCorteFrontend();
    const corteRegular = new Date(`${anioCorte}-03-31T23:59:59`);
    const hoy = new Date();
    const fechaInicio = anioSeleccionado.fecha_inicio ? new Date(`${String(anioSeleccionado.fecha_inicio).slice(0, 10)}T00:00:00`) : null;
    const fechaFin = anioSeleccionado.fecha_fin ? new Date(`${String(anioSeleccionado.fecha_fin).slice(0, 10)}T23:59:59`) : null;
    const estaEnCurso = fechaInicio && fechaFin && hoy >= fechaInicio && hoy <= fechaFin;
    const pasoCorteRegular = hoy > corteRegular;
    if (estadoOperativo === 'En curso' || (estaEnCurso && pasoCorteRegular)) return ['Traslado','Reingreso','Regularización'];
    return ['Nuevo','Traslado','Reingreso','Continuidad interna','Renovación','Renovación con cambio de sede'];
  }, [anioSeleccionado, anioId, anios]);

  useEffect(() => {
    if (!anioId) return;
    if (!tiposIngresoPermitidos.length) { setTipoIngreso(''); return; }
    if (!tiposIngresoPermitidos.includes(tipoIngreso)) { setTipoIngreso(tiposIngresoPermitidos[0]); setExcepcionTraslado(false); }
  ...
  const avisoPeriodoMatricula = useMemo(() => {
    ...
    const estadoOperativo = getEstadoOperativoAnioFrontend(anioSeleccionado);
    const hoy = new Date();
    const fechaInicio = anioSeleccionado.fecha_inicio ? new Date(anioSeleccionado.fecha_inicio) : null;
    const fechaFin = anioSeleccionado.fecha_fin ? new Date(anioSeleccionado.fecha_fin) : null;
    const anioCorte = getAnioCorteFrontend();
    const corteRegular = new Date(`${anioCorte}-03-31T23:59:59`);
    if (estadoOperativo === 'Cerrado' || (fechaFin && hoy > fechaFin)) return { bloquea: true, tipo: 'error', texto: 'El año lectivo seleccionado está cerrado o vencido. Selecciona un año lectivo vigente o crea uno nuevo en Configuración > Años lectivos.' };
    if (estadoOperativo === 'Planificación') {
      if (!['Reserva','Renovación','Renovación con cambio de sede'].includes(tipoIngreso)) return { bloquea: true, tipo: 'warning', texto: 'Este año lectivo todavía está en planificación. Solo puedes registrar reservas o renovaciones anticipadas.' };
      const esRenovacion = ['Renovación','Renovación con cambio de sede'].includes(tipoIngreso);
      return { bloquea: false, tipo: 'info', texto: esRenovacion ? 'Se registrará como pre-matrícula anticipada para el siguiente año.' : 'Se registrará como reserva. No se generará cobro ni activación hasta que el año lectivo pase a Matrícula abierta o En curso.' };
    }
    const tiposPermitidosEnCurso = ['Traslado','Reingreso','Regularización'];
    const estaEnCurso = fechaInicio && fechaFin && hoy >= fechaInicio && hoy <= fechaFin;
    const pasoCorteRegular = hoy > corteRegular;
    if (estadoOperativo === 'En curso' || (estaEnCurso && pasoCorteRegular)) {
      if (!tiposPermitidosEnCurso.includes(tipoIngreso)) return { bloquea: true, tipo: 'warning', texto: 'La matrícula regular de este año ya está cerrada. En periodo en curso solo se permiten Traslado, Reingreso o Regularización autorizada.' };
      return { bloquea: false, tipo: 'info', texto: 'Periodo en curso: se permitirá continuar porque el tipo de ingreso corresponde a traslado, reingreso o regularización autorizada.' };
    }
    if (tipoIngreso === 'Reserva') return { bloquea: true, tipo: 'warning', texto: 'El tipo Reserva debe usarse para años futuros o en planificación.' };
    return null;
  ...
  const reglaEdadSeleccionada = (): ReglaEdad | null => {
    ...
    if (nivel.includes('inicial')) {
      ...
      if (edad >= 3 && edad <= 5) return { edad, permiteExcepcionTraslado: false, label: `Inicial ${edad} años` };
    }
    if (nivel.includes('primaria')) {
      const gradoNumero = grado.includes('primer')||grado.includes('1') ? 1 : grado.includes('segundo')||grado.includes('2') ? 2 : grado.includes('tercer')||grado.includes('3') ? 3 : grado.includes('cuarto')||grado.includes('4') ? 4 : grado.includes('quinto')||grado.includes('5') ? 5 : grado.includes('sexto')||grado.includes('6') ? 6 : null;
      if (gradoNumero) return { edad: 5 + gradoNumero, permiteExcepcionTraslado: gradoNumero >= 2, label: `${gradoNumero}.° de primaria` };
    }
    return null;
  };
  ...
  const errorEdadNormativa = useMemo(() => {
    ...
    return `El alumno no cumple la edad para ${regla.label}. Debe tener ${regla.edad} años cumplidos al 31 de marzo de ${anioCorte}. Edad al corte: ${edadCorte.texto}.`;
  ...
  const avisoEdadFichaAlumno = useMemo(() => {
    ...
    if (!edad) return null;
    if (edad.anios < 3) return `Aviso: para el año lectivo ${anioCorte}, el alumno tendría ${edad.texto} al 31 de marzo.`;
    return null;
  ...
  const matriculaActiva = useMemo(() => {
    ...
    const matriculasBloqueantes = estudiante.matriculas.filter((m) =>
      estadosMatriculaBloqueantes.includes(m.estado_matricula) && getAnioCorteMatriculaFrontend(m) === anioDestino
    );
    if (!matriculasBloqueantes.length) return null;
    if (activeScope.tipo === 'colegio') {
      const mismaSede = matriculasBloqueantes.find((m) => m.id_colegio === activeScope.id_colegio);
      if (mismaSede) return mismaSede;
    }
    if (colegioDestinoId) {
      const mismaSedeDestino = matriculasBloqueantes.find((m) => m.id_colegio === colegioDestinoId);
  ...
  const alertaEdad = useMemo(() => {
    ...
    if (edad < 0) return 'La fecha de nacimiento del alumno es futura o inválida.';
    const nivel = seccionSeleccionada.grado?.nivel?.nombre_nivel?.toLowerCase() || '';
  ...
  const formatMatriculaActiva = (matricula: any) => {
    ...
    if (estado === 'Reserva') return `Este alumno ya tiene una reserva registrada en ${colegio}, ${grado} "${letra}" · ${nivel}, ${anio}.`;
    if (estado === 'Pre-matriculado') return `Este alumno ya está pre-matriculado en ${colegio}, ${grado} "${letra}" · ${nivel}, ${anio}.`;
    if (estado === 'Activo') return `Este alumno ya tiene una matrícula activa en ${colegio}, ${grado} "${letra}" · ${nivel}, ${anio}.`;
    return `Este alumno ya figura como ${estado} en ${colegio}, ${grado} "${letra}" · ${nivel}, ${anio}.`;
  };

  const irADetalleMatriculaActiva = (matricula: any) => {
    if (!matricula?.id_matricula) return;
    const params = new URLSearchParams();
    params.set('matricula_id', String(matricula.id_matricula));
    if (matricula.id_colegio) params.set('colegio_id', String(matricula.id_colegio));
    navigate(`/matricula/historial?${params.toString()}`);
  };

  const mensajeValidacionMatricula = useMemo(() => {
    if (colegioDestinoRequerido) return { tipo: 'info' as const, texto: 'Selecciona el colegio destino. Estás trabajando con todos los colegios.' };
    if (!alumno) return { tipo: 'info' as const, texto: 'Busca o registra primero al alumno que deseas matricular.' };
    if (!apoderados.length) return { tipo: 'warning' as const, texto: 'El alumno debe tener al menos un apoderado vinculado antes de registrar la matrícula.' };
    if (!anioId) return { tipo: 'info' as const, texto: 'Selecciona el año lectivo de la matrícula.' };
    if (!seccionId) return { tipo: 'info' as const, texto: 'Selecciona el grado y sección donde se registrará al alumno.' };
    if (avisoPeriodoMatricula?.bloquea) return { tipo: avisoPeriodoMatricula.tipo as 'error'|'warning'|'info', texto: avisoPeriodoMatricula.texto };
    if (errorEdadNormativa) return { tipo: 'error' as const, texto: errorEdadNormativa };
    if (avisoPeriodoMatricula) return { tipo: avisoPeriodoMatricula.tipo as 'error'|'warning'|'info', texto: avisoPeriodoMatricula.texto };
    return null;
  }, [colegioDestinoRequerido, alumno, apoderados.length, anioId, seccionId, avisoPeriodoMatricula, errorEdadNormativa]);
  ...
  useEffect(() => {
    ...
    setAlumno(null); setApoderados([]); setSeccionId(''); setAnioId(''); setMensaje(null);
  ...
  const fetchBase = async () => {
    ...
    try {
      const [ultimasRes, aniosRes] = await Promise.all([
        axios.get(`/api/academicos/matriculas/ultimas${colegioDestinoQuery}`, { headers: { Authorization: `Bearer ${token}` } }),
        axios.get(`/api/academicos/anios${colegioDestinoQuery}`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      const aniosData: Anio[] = aniosRes.data || [];
      setUltimas((ultimasRes.data || []).slice(0, 5));
      setAnios(aniosData);
      const estadoNorm = (estado?: string) => String(estado||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
      const fechaNoVencida = (a: Anio) => { if (!a.fecha_fin) return true; const f = new Date(`${String(a.fecha_fin).slice(0,10)}T23:59:59`); return Number.isNaN(f.getTime()) || f >= new Date(); };
      const aniosRegistrables = aniosData.filter((a) => !['cerrado','archivado'].includes(estadoNorm(a.estado)) && fechaNoVencida(a));
      const activo = aniosRegistrables.find((a) => estadoNorm(a.estado).includes('matricula')) || aniosRegistrables.find((a) => estadoNorm(a.estado) === 'abierto') || aniosRegistrables.find((a) => estadoNorm(a.estado).includes('curso')) || aniosRegistrables.find((a) => estadoNorm(a.estado) === 'activo') || aniosRegistrables.find((a) => estadoNorm(a.estado).includes('planificacion')) || aniosRegistrables[0];
      const resolved = activo ? activo.id_anio : '';
      setAnioId((current) => current || resolved);
      if (resolved) await fetchSecciones(Number(resolved));
    } catch { setUltimas([]); setAnios([]); setSecciones([]); }
  ...
  const fetchSecciones = async (idAnio: number) => {
    ...
    try {
      const res = await axios.get(`/api/academicos/secciones${query}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const abrirDetalleMatricula = async (idMatricula: number) => {
    if (!token) return;
    setDetalleOpen(true); setDetalleLoading(true); setDetalleMatricula(null); setCronogramaOpen(false);
    try {
      const res = await axios.get(`/api/academicos/matriculas/${idMatricula}/detalle${colegioDestinoQuery}`, { headers: { Authorization: `Bearer ${token}` } });
      setDetalleMatricula(res.data);
    } catch (error: any) {
      setMensaje(error.response?.data?.message || 'No se pudo cargar el detalle de matrícula.');
      setDetalleOpen(false);
    } finally { setDetalleLoading(false); }
  };

  const imprimirDetalleMatricula = () => window.print();
  ...
  const apoderadosDesdeAlumno = (alumnoData: Alumno): Apoderado[] => {
    ...
    return estudianteData.apoderados.map((relacion) => ({
      ...
      apoderado: { id_persona: relacion.apoderado.id_persona, ocupacion: relacion.apoderado.ocupacion },
  ...
  const buscarAlumnoPorDni = async (dniBusqueda: string) => {
    ...
    setBuscandoAlumno(true); setMensaje(null); setApoderados([]);
    try {
      const query = puedeVerConsolidado ? '&scope=all' : colegioDestinoQuery ? `&${colegioDestinoQuery.replace('?','')}` : '';
      const res = await axios.get(`/api/academicos/alumnos/buscar?dni=${dniBusqueda.trim()}${query}`, { headers: { Authorization: `Bearer ${token}` } });
      setAlumno(res.data);
      setApoderados(apoderadosDesdeAlumno(res.data));
      setSeccionId(''); setNivelFiltro(''); setGradoFiltro(''); setMensaje(null);
    } catch (err: any) {
      setAlumno(null);
  ...
  const buscarApoderadoPorDni = async (dniBusqueda: string) => {
    if (!token || !dniBusqueda.trim()) return;
    setBuscandoApoderado(true); setMensaje(null);
    try {
      const res = await axios.get(`/api/academicos/apoderados/buscar?dni=${dniBusqueda.trim()}`, { headers: { Authorization: `Bearer ${token}` } });
      ...
      return null;
    } finally { setBuscandoApoderado(false); }
  };

  const buscarApoderado = () => buscarApoderadoPorDni(apoderadoDni);

  const agregarApoderado = async (apoderado: Apoderado) => {
    if (!estudiante?.id_persona || !token) { setMensaje('Primero debes buscar o registrar un alumno.'); return; }
    if (apoderados.some((item) => item.id_persona === apoderado.id_persona)) { setMensaje('Este apoderado ya está vinculado al alumno.'); return; }
    const parentescoSeleccionado = apoderado.parentesco || parentesco || 'Apoderado';
    try {
      await axios.post(`/api/academicos/alumnos/${estudiante.id_persona}/apoderados`, { id_apoderado: apoderado.id_persona, parentesco: parentescoSeleccionado }, { headers: { Authorization: `Bearer ${token}` } });
      setApoderados([...apoderados, { ...apoderado, parentesco: parentescoSeleccionado }]);
      setMensaje(null);
      showToast({ type: 'success', title: 'Apoderado vinculado', message: `${parentescoSeleccionado} agregado correctamente.` });
      setApoderadoEncontrado(null); setApoderadoDni(''); setParentesco('Madre');
    } catch (error: any) { setMensaje(error.response?.data?.message || 'No se pudo vincular el apoderado.'); }
  };

  const crearPersona = async (tipo: 'alumno' | 'apoderado') => {
    if (!token) return;
    const form = tipo === 'alumno' ? formAlumno : formApoderado;
    if (!form.dni || !form.nombres || !form.apellido_paterno || !form.apellido_materno) { setErrorPersona('Completa DNI, nombres y apellidos.'); return; }
    if (tipo === 'alumno') { const errorFecha = validarFechaNacimientoFrontend(form.fecha_nacimiento); if (errorFecha) { setErrorPersona(errorFecha); return; } }
    setSavingPersona(true); setErrorPersona(null);
    try {
      const res = await axios.post(tipo === 'alumno' ? '/api/academicos/alumnos' : '/api/academicos/apoderados', { ...form, pais: form.pais || 'Perú' }, { headers: { Authorization: `Bearer ${token}` } });
      if (tipo === 'alumno') {
        setDni(form.dni); setModalAlumno(false); setFormAlumno(emptyAlumno);
        showToast({ type: 'success', title: 'Alumno registrado', message: 'La ficha del alumno se guardó correctamente.' });
        await buscarAlumnoPorDni(form.dni);
      } else {
        const parentescoNuevo = parentesco || 'Apoderado';
        const apoderadoCreado: Apoderado = { id_persona: res.data?.apoderado?.id_persona || res.data?.persona?.id_persona, dni: res.data?.persona?.dni || form.dni, nombres: res.data?.persona?.nombres || form.nombres, apellido_paterno: res.data?.persona?.apellido_paterno || form.apellido_paterno, apellido_materno: res.data?.persona?.apellido_materno || form.apellido_materno, telefono: res.data?.persona?.telefono || form.telefono, correo: res.data?.persona?.correo || form.correo, direccion: res.data?.persona?.direccion || form.direccion, pais: res.data?.persona?.pais || form.pais, departamento: res.data?.persona?.departamento || form.departamento, provincia: res.data?.persona?.provincia || form.provincia, distrito: res.data?.persona?.distrito || form.distrito, parentesco: parentescoNuevo, apoderado: { id_persona: res.data?.apoderado?.id_persona || res.data?.persona?.id_persona, ocupacion: res.data?.apoderado?.ocupacion || form.ocupacion } };
        setApoderadoDni(form.dni); setModalApoderado(false); setFormApoderado(emptyApoderado);
        showToast({ type: 'success', title: 'Apoderado registrado', message: 'La ficha del apoderado se guardó correctamente.' });
        if (estudiante?.id_persona) await agregarApoderado(apoderadoCreado);
        else setApoderadoEncontrado(apoderadoCreado);
      }
    } catch (err: any) { setErrorPersona(err.response?.data?.message || 'No se pudo guardar el registro.'); }
  ...
  const editarAlumno = async () => {
    ...
    if (errorFecha) { setErrorPersona(errorFecha); return; }
    setSavingPersona(true); setErrorPersona(null);
    try {
      const res = await axios.put(`/api/academicos/alumnos/${estudiante.id_persona}`, { ...formAlumno, pais: formAlumno.pais || 'Perú' }, { headers: { Authorization: `Bearer ${token}` } });
      setAlumno(res.data); setApoderados(apoderadosDesdeAlumno(res.data)); setModalEditarAlumno(false); setMensaje(null);
      showToast({ type: 'success', title: 'Alumno actualizado', message: 'Los datos del alumno se actualizaron correctamente.' });
    } catch (err: any) { setErrorPersona(err.response?.data?.message || 'No se pudo actualizar el alumno.'); }
    finally { setSavingPersona(false); }
  };

  const limpiarFlujoMatricula = () => {
    setDni(''); setAlumno(null); setApoderados([]); setApoderadoDni(''); setApoderadoEncontrado(null);
    setParentesco('Madre'); setSeccionId(''); setNivelFiltro(''); setGradoFiltro(''); setExcepcionTraslado(false); setMensaje(null);
    setTipoIngreso('Nuevo'); setColegioProcedencia(''); setCodigoModularProcedencia(''); setGradoProcedencia(''); setObservacionProcedencia('');
  };

  const revisarMatricula = () => {
    if (!alumno || !estudiante) return setMensaje('Primero busca o registra un alumno.');
    if (colegioDestinoRequerido) { setMensaje('Selecciona el colegio destino.'); return; }
    if (mensajeValidacionMatricula && (mensajeValidacionMatricula.tipo === 'error' || mensajeValidacionMatricula.tipo === 'warning')) { setMensaje(mensajeValidacionMatricula.texto); return; }
  ...
  const registrarMatricula = async () => {
    ...
    setMatriculando(true); setMensaje(null);
    try {
      const res = await axios.post(`/api/academicos/matriculas${colegioDestinoQuery}`, { id_estudiante: estudiante.id_persona, id_anio: Number(anioId), id_seccion: Number(seccionId), id_colegio: Number(colegioDestinoId || activeColegio?.id_colegio), apoderados: apoderados.map((a) => ({ id_apoderado: a.id_persona, parentesco: a.parentesco || 'Apoderado' })), excepcion_traslado: excepcionTraslado, tipo_ingreso: tipoIngreso, colegio_procedencia: colegioProcedencia, codigo_modular_procedencia: codigoModularProcedencia, grado_procedencia: gradoProcedencia, observacion_procedencia: observacionProcedencia }, { headers: { Authorization: `Bearer ${token}` } });
      const estadoGuardado = res.data?.estado_matricula || tipoIngreso;
      showToast({ type: 'success', title: estadoGuardado === 'Reserva' || tipoIngreso === 'Reserva' ? 'Reserva registrada' : 'Pre-matrícula registrada', message: estadoGuardado === 'Reserva' || tipoIngreso === 'Reserva' ? 'La reserva se guardó correctamente.' : 'La pre-matrícula se guardó correctamente.' });
      setConfirmOpen(false); limpiarFlujoMatricula(); await fetchBase();
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || 'No se pudo registrar la matrícula.';
      setMensaje(errorMessage); showToast({ type: 'error', title: 'No se pudo registrar', message: errorMessage }); setConfirmOpen(false);
  ...
  return (
    <div className="w-full space-y-6 animate-page-soft">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.92fr_1.65fr]">
        ...
        <section className="space-y-5">
          ...
          <Card icon={Search} title="Buscar alumno" subtitle="Ingresa el DNI para revisar su ficha.">
            <div className="flex gap-2">
              ...
              >
                {buscandoAlumno ? <Loader2 size={15} className="animate-spin" /> : <Search size={15} />}
                Buscar
              </button>
            </div>
            <button
              type="button"
              onClick={() => { setFormAlumno({ ...emptyAlumno, dni }); setErrorPersona(null); setModalAlumno(true); }}
              className="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 bg-white px-4 text-sm font-semibold text-slate-500 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700"
            >
              <UserPlus size={15} />
              Nuevo alumno
            </button>
          ...
          <Card
            ...
            action={
              <button
                ...
                className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-500 transition hover:bg-slate-50"
              >
                Ver todas
              </button>
            }
              ...
              <div className="space-y-2">
                {ultimas.slice(0, 5).map((matricula) => {
                  const registrador = matricula.registrado_por?.persona
                    ...
                    : 'No registrado';
                  const estadoCls = getEstadoCls(matricula.estado_matricula);
                  return (
                    <button
                      key={matricula.id_matricula}
                      type="button"
                      onClick={() => abrirDetalleMatricula(matricula.id_matricula)}
                      className="group w-full rounded-2xl bg-slate-50 p-4 text-left ring-1 ring-slate-100 transition hover:bg-white hover:shadow-sm"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-bold text-slate-800">
                            {getCodigoMatricula(matricula)} · {matricula.estudiante.persona.nombres}{' '}
                            {matricula.estudiante.persona.apellido_paterno}
                          </p>
                          <p className="mt-0.5 text-xs text-slate-400">
                            {matricula.seccion.grado.nombre_grado} &quot;{matricula.seccion.letra}&quot;{' '}
                            · {matricula.seccion.grado.nivel?.nombre_nivel || 'Nivel'}
                          </p>
                          <p className="mt-0.5 text-xs text-slate-400">{formatFechaHora(matricula.fecha_matricula)}</p>
                          <p className="mt-0.5 truncate text-xs text-slate-500">Por: {registrador}</p>
                        </div>
                        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${estadoCls}`}>
        ...
        <section className="space-y-5">
          {alumno ? (
            <div className="animate-content-soft overflow-hidden rounded-[28px] border border-slate-100 bg-white shadow-sm shadow-slate-100/80">
              {/* Ficha header */}
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/60 px-5 py-3.5">
                <div className="flex min-w-0 items-center gap-3">
                  {(() => {
                    ...
                    const initials = getInitials(alumno.nombres);
                    return (
                      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-xs font-black ${color}`}>
                        {initials}
                      </div>
                    );
                  })()}
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 flex flex-wrap gap-1.5 text-[11px] text-slate-400">
                      <span className="rounded-full bg-white px-2 py-0.5 ring-1 ring-slate-100">
                        DNI: {alumno.dni}
                      </span>
                      <span className="rounded-full bg-white px-2 py-0.5 ring-1 ring-slate-100">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  {matriculaActiva ? (
                    <Badge tone="amber">{matriculaActiva.estado_matricula}</Badge>
                  ) : (
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setFormAlumno({ dni: alumno.dni||'', nombres: alumno.nombres||'', apellido_paterno: alumno.apellido_paterno||'', apellido_materno: alumno.apellido_materno||'', fecha_nacimiento: alumno.fecha_nacimiento ? alumno.fecha_nacimiento.slice(0,10) : '', genero: alumno.genero||'', telefono: alumno.telefono||'', correo: alumno.correo||'', direccion: alumno.direccion||'', pais: alumno.pais||'Perú', departamento: alumno.departamento||'', provincia: alumno.provincia||'', distrito: alumno.distrito||'' });
                      setErrorPersona(null); setModalEditarAlumno(true);
                    }}
                    className="inline-flex h-8 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-600 transition hover:border-slate-300 hover:bg-slate-50"
                  >
                    <ArrowRight size={12} />
                    Editar
                  </button>
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                    ...
                    <div className="mt-3 space-y-2">
                      {estudiante.matriculas.map((m) => (
                        <div key={m.id_matricula} className="flex items-center justify-between gap-3 rounded-xl bg-white p-3 ring-1 ring-slate-100">
                          <div>
                            <p className="text-sm font-bold text-slate-800">
                              {m.seccion?.grado?.nombre_grado || 'Grado'} &quot;{m.seccion?.letra || '-'}&quot;
                            </p>
                            <p className="mt-0.5 text-xs text-slate-400">
                              {m.colegio?.nombre || 'Colegio'} · {m.anio?.nombre_anio || 'Año lectivo'}
                            </p>
                          </div>
                          <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${getEstadoCls(m.estado_matricula)}`}>
                            {m.estado_matricula}
                          </span>
                        </div>
                      ))}
            ...
            <div className="overflow-hidden rounded-[28px] border border-slate-100 bg-white shadow-sm shadow-slate-100/80">
              <div className="flex min-h-[260px] flex-col items-center justify-center gap-3 p-6 text-center">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-700">Busca un alumno por DNI</h3>
                  <p className="mt-1 max-w-xs text-xs text-slate-400">
                    Al encontrarlo podrás vincular apoderados y registrar la pre-matrícula.
                  </p>
                </div>
              </div>
            </div>
          ...
          {alumno && matriculaActiva && (
            <div className="animate-content-soft">
              <Card
                icon={AlertCircle}
                title={matriculaActiva.estado_matricula === 'Reserva' ? 'Alumno con reserva registrada' : matriculaActiva.estado_matricula === 'Pre-matriculado' ? 'Alumno pre-matriculado' : matriculaActiva.estado_matricula === 'Activo' ? 'Alumno con matrícula activa' : 'Matrícula existente'}
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <button
                    ...
                    className="h-10 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:bg-slate-50"
                  >
                    Ver detalle
                  </button>
                  <button
                    type="button"
                    onClick={() => { setDni(''); setAlumno(null); setApoderados([]); setMensaje(null); }}
              ...
              <Card icon={ShieldCheck} title="Apoderados" subtitle="Debes vincular al menos un apoderado.">
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && buscarApoderado()}
                    placeholder="DNI del apoderado"
                    className={inputClass}
                  />
                  <select value={parentesco} onChange={(e) => setParentesco(e.target.value)} className="h-11 rounded-2xl border border-slate-200 bg-slate-50/70 px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-accent-300 focus:bg-white focus:ring-4 focus:ring-accent-100">
                  ...
                  <button
                    ...
                    className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {buscandoApoderado ? <Loader2 size={15} className="animate-spin" /> : <Search size={15} />}
                    Buscar
                  </button>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => { setFormApoderado({ ...emptyApoderado }); setErrorPersona(null); setModalApoderado(true); }}
                  className="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 bg-white px-4 text-sm font-semibold text-slate-500 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700"
                >
                  <UserPlus size={15} />
                  Nuevo apoderado
                ...
                {apoderadoEncontrado && (
                  <div className="animate-content-soft mt-3 rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-100">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-bold text-slate-800">
                          {apoderadoEncontrado.nombres} {apoderadoEncontrado.apellido_paterno} {apoderadoEncontrado.apellido_materno}
                        </p>
                        <p className="mt-0.5 text-xs text-slate-400">
                          DNI {apoderadoEncontrado.dni} · {apoderadoEncontrado.telefono || 'Sin teléfono'} · {apoderadoEncontrado.distrito || 'Sin distrito'}
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={() => agregarApoderado({ ...apoderadoEncontrado, parentesco })}
                        className="shrink-0 h-9 rounded-xl bg-accent-500 px-3 text-xs font-bold text-white transition hover:bg-accent-600"
                      >
                ...
                {apoderados.length > 0 && (
                  <div className="mt-4 space-y-2">
                    <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 ring-1 ring-emerald-200">
                      Apoderados cargados desde la ficha del alumno
                    </div>
                    {apoderados.map((a) => {
                      const color = getAvatarColor(a.nombres);
                      const initials = getInitials(a.nombres);
                      return (
                        <div key={a.id_persona} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-100">
                          <div className="flex min-w-0 items-center gap-2.5">
                            <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-[11px] font-black ${color}`}>
                              {initials}
                            </div>
                            <div className="min-w-0">
                              <p className="truncate text-sm font-bold text-slate-800">
                              ...
                              <p className="mt-0.5 truncate text-xs text-slate-400">
                                {a.parentesco} · DNI {a.dni} · {a.telefono || 'Sin teléfono'}
                              </p>
                            </div>
                          </div>
                          <button
                            ...
                            onClick={async () => {
                              if (!token || !estudiante?.id_persona) return;
                              try {
                                await axios.delete(`/api/academicos/alumnos/${estudiante.id_persona}/apoderados/${a.id_persona}`, { headers: { Authorization: `Bearer ${token}` } });
                              ...
                              } catch (err: any) { setMensaje(err.response?.data?.message || 'No se pudo desvincular el apoderado.'); }
                            }}
                            className="shrink-0 rounded-lg px-2.5 py-1 text-xs font-bold text-rose-500 transition hover:bg-rose-50 hover:text-rose-700"
                          >
                            Quitar
                          </button>
                        </div>
              ...
              <Card icon={GraduationCap} title="Registrar pre-matrícula" subtitle="Filtra por nivel y grado para elegir sección.">
                <div className="grid gap-4 xl:grid-cols-3">
                  <label>
                    ...
                    <select value={anioId} onChange={(e) => { setAnioId(e.target.value ? Number(e.target.value) : ''); setExcepcionTraslado(false); }} className={selectClass}>
                      <option value="">Selecciona año</option>
                      {aniosDisponibles.map((a) => <option key={a.id_anio} value={a.id_anio}>{a.nombre_anio} · {a.estado}</option>)}
                    </select>
                    {aniosDisponibles.length === 0 && <p className="mt-2 text-xs font-semibold text-rose-500">No hay años lectivos disponibles. Crea o abre uno desde Configuración &gt; Años lectivos.</p>}
                  </label>
                  <label>
                    <Label>Nivel</Label>
                ...
                <div className="mt-5 grid max-h-[310px] gap-3 overflow-y-auto pr-1 md:grid-cols-2">
                  {seccionesFiltradas.map((s) => {
                    ...
                    const porcentaje = s.capacidad > 0 ? Math.min(100, Math.round((s.matriculados / s.capacidad) * 100)) : 0;
                    return (
                      <button
                        key={s.id_seccion}
                        type="button"
                        disabled={sinCupos}
                        onClick={() => { setSeccionId(selected ? '' : s.id_seccion); setExcepcionTraslado(false); }}
                        className={cx(
                          'rounded-2xl border p-4 text-left transition',
                          selected ? 'border-accent-300 bg-accent-50 ring-4 ring-accent-100' : 'border-slate-100 bg-slate-50 hover:bg-white hover:shadow-sm',
                          sinCupos && 'cursor-not-allowed opacity-50',
                        )}
                      >
                        <div className="flex justify-between gap-3">
                          <div>
                            <p className="text-sm font-bold text-slate-800">{s.grado.nombre_grado} &quot;{s.letra}&quot;</p>
                            <p className="mt-0.5 text-xs text-slate-400">{s.grado.nivel?.nombre_nivel || 'Nivel'} · {s.capacidad} cupos</p>
                          </div>
                        ...
                        <p className="mt-2 text-xs text-slate-400">{s.matriculados} registrados · {s.disponibles} disponibles</p>
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="mt-5 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <label>
                      ...
                      <select value={tipoIngreso} onChange={(e) => setTipoIngreso(e.target.value)} className={selectClass} disabled={tiposIngresoPermitidos.length === 0}>
                        {tiposIngresoPermitidos.length === 0 ? <option value="">Sin opciones disponibles</option> : tiposIngresoPermitidos.map((tipo) => <option key={tipo} value={tipo}>{tipo}</option>)}
                      </select>
                      {anioSeleccionado && <p className="mt-2 text-xs text-slate-400">{getEstadoOperativoAnioFrontend(anioSeleccionado) === 'Planificación' ? 'Año en planificación: solo permite reservas.' : getEstadoOperativoAnioFrontend(anioSeleccionado) === 'En curso' ? 'Año en curso: solo permite traslado, reingreso o regularización.' : 'Matrícula abierta: permite ingreso regular.'}</p>}
                      {['Renovación','Renovación con cambio de sede'].includes(tipoIngreso) && <p className="mt-2 rounded-2xl bg-sky-50 px-4 py-3 text-xs font-semibold leading-5 text-sky-700 ring-1 ring-sky-100">Este tipo se usa para alumnos vigentes que desean asegurar matrícula del siguiente año.</p>}
                    </label>
                    {(tipoIngreso === 'Traslado' || tipoIngreso === 'Reingreso') && (
                      <>
                        <label><Label>Colegio de procedencia</Label><input value={colegioProcedencia} onChange={(e) => setColegioProcedencia(e.target.value)} placeholder="Nombre del colegio anterior" className={inputClass} /></label>
                        <label><Label>Código modular</Label><input value={codigoModularProcedencia} onChange={(e) => setCodigoModularProcedencia(e.target.value)} placeholder="Opcional" className={inputClass} /></label>
                        <label><Label>Grado procedencia</Label><input value={gradoProcedencia} onChange={(e) => setGradoProcedencia(e.target.value)} placeholder="Ej. Inicial 4 años, 2do primaria" className={inputClass} /></label>
                      </>
                    )}
                    <label className="md:col-span-2">
                      <Label>Observación</Label>
                      <textarea value={observacionProcedencia} onChange={(e) => setObservacionProcedencia(e.target.value)} placeholder="Información adicional sobre ingreso, traslado o continuidad" className="min-h-24 w-full rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-accent-300 focus:bg-white focus:ring-4 focus:ring-accent-100" />
                    </label>
                  </div>
                </div>

                {seccionSeleccionada && reglaEdadSeleccionada()?.permiteExcepcionTraslado && (
                  <label className="mt-4 flex items-start gap-3 rounded-2xl bg-amber-50 p-4 text-sm font-semibold text-amber-700 ring-1 ring-amber-200">
                ...
                {mensajeValidacionMatricula && (
                  <div className={cx('mt-4 rounded-2xl px-4 py-3 text-sm font-semibold ring-1', mensajeValidacionMatricula.tipo === 'error' ? 'bg-rose-50 text-rose-700 ring-rose-200' : mensajeValidacionMatricula.tipo === 'warning' ? 'bg-amber-50 text-amber-700 ring-amber-200' : 'bg-sky-50 text-sky-700 ring-sky-200')}>
                    {mensajeValidacionMatricula.texto}
                  </div>
                )}

                {mensaje && (
                  <div className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-600 ring-1 ring-slate-100">
                    {mensaje}
                  </div>
                )}
                ...
                <button
                  ...
                  className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-accent-500 px-5 text-sm font-bold text-white shadow-lg shadow-accent-500/20 transition hover:bg-accent-600 disabled:cursor-not-allowed disabled:opacity-50"
                >
      ...
      {confirmOpen && alumno && seccionSeleccionada && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 px-4 backdrop-blur-sm animate-modal-overlay-in">
          <div className="w-full max-w-2xl overflow-hidden rounded-[28px] bg-white shadow-2xl ring-1 ring-slate-200 animate-modal-panel-in">
            <ModalHead title="Revisar pre-matrícula" subtitle="Verifica los datos antes de guardar." onClose={() => setConfirmOpen(false)} />
            <div className="space-y-4 p-6">
              {alertaEdad && (
                <div className="rounded-2xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700 ring-1 ring-amber-200">
              ...
              <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">Apoderados</p>
                <div className="mt-3 space-y-2">
                  {apoderados.map((a) => (
                    <div key={a.id_persona} className="rounded-xl bg-white px-3 py-2 text-sm font-semibold text-slate-700 ring-1 ring-slate-100">
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-slate-100 p-6 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setConfirmOpen(false)} className="h-10 rounded-xl border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Corregir</button>
              <button type="button" onClick={registrarMatricula} disabled={matriculando} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-accent-500 px-5 text-sm font-bold text-white transition hover:bg-accent-600 disabled:opacity-50">
                {matriculando ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />}
      ...
      {detalleOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 px-4 backdrop-blur-sm animate-modal-overlay-in">
          <div className="w-full max-w-4xl overflow-hidden rounded-[28px] bg-white shadow-2xl ring-1 ring-slate-200 animate-modal-panel-in">
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-6">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-400">Información académica, apoderados y cronograma generado.</p>
              </div>
              <div className="flex items-center gap-2">
                <button type="button" onClick={imprimirDetalleMatricula} className="inline-flex h-9 items-center justify-center rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600 transition hover:bg-slate-50">Imprimir / PDF</button>
                <button type="button" onClick={() => setDetalleOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200"><X size={15} /></button>
              </div>
            </div>
                ...
                <div className="space-y-5">
                    ...
                    <div className="space-y-3">
                      {detalleMatricula.estudiante?.apoderados?.length ? (
                        detalleMatricula.estudiante.apoderados.map((relacion: any) => (
                          <div key={relacion.id_apoderado} className="rounded-2xl bg-white p-4 ring-1 ring-slate-100">
                            <p className="text-sm font-bold text-slate-800">{relacion.parentesco}: {relacion.apoderado.persona.nombres} {relacion.apoderado.persona.apellido_paterno}</p>
                            <div className="mt-3 grid gap-2 text-xs text-slate-500 md:grid-cols-2">
                              <p><span className="text-slate-400">DNI:</span> {relacion.apoderado.persona.dni || '—'}</p>
                              <p><span className="text-slate-400">Número:</span> {relacion.apoderado.persona.telefono || '—'}</p>
                              <p><span className="text-slate-400">Correo:</span> {relacion.apoderado.persona.correo || '—'}</p>
                              <p><span className="text-slate-400">Distrito:</span> {relacion.apoderado.persona.distrito || '—'}</p>
                              <p><span className="text-slate-400">Departamento:</span> {relacion.apoderado.persona.departamento || '—'}</p>
                              <p><span className="text-slate-400">Dirección:</span> {relacion.apoderado.persona.direccion || '—'}</p>
                            </div>
                          </div>
                        ))
                      ) : (
                  ...
                  <div className="rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-100">
                    <button type="button" onClick={() => setCronogramaOpen(!cronogramaOpen)} className="flex w-full items-center justify-between text-left">
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-[0.15em] text-slate-500">Cronograma de pagos</h4>
                        <p className="mt-1 text-xs text-slate-400">{detalleMatricula.cronogramas?.length || 0} conceptos generados</p>
                      </div>
                      <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-500 transition hover:bg-slate-50">
                      ...
                      <div className="mt-4 space-y-2">
                        {detalleMatricula.cronogramas?.length ? (
                          detalleMatricula.cronogramas.map((item: any) => (
                            <div key={item.id_cronograma} className="flex flex-col gap-2 rounded-2xl bg-white p-3 ring-1 ring-slate-100 sm:flex-row sm:items-center sm:justify-between">
                              <div>
                                <p className="text-sm font-bold text-slate-800">{item.concepto.nombre_concepto}</p>
                                <p className="mt-0.5 text-xs text-slate-400">Vencimiento: {new Date(item.fecha_vencimiento).toLocaleDateString('es-PE')} · Monto: {formatMoney(item.concepto.monto_base)}</p>
                              </div>
                              <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold ${getEstadoCls(item.estado_pago)}`}>
                                {item.estado_pago}
                              </span>
                            </div>
                          ))
                        ) : (
                          <p className="text-sm text-slate-400">No hay conceptos generados.</p>
                        )}
                      </div>
                    )}
                  </div>
    ...
    <div className="animate-content-soft overflow-hidden rounded-[28px] border border-slate-100 bg-white shadow-sm shadow-slate-100/80">
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/60 px-5 py-3.5">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-accent-50 text-accent-600 ring-1 ring-accent-100">
            <Icon size={15} />
          </div>
          <div>
...
function Badge({ tone, children }: { tone: 'emerald' | 'amber'; children: string }) {
  const cls = tone === 'emerald' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' : 'bg-amber-50 text-amber-700 ring-1 ring-amber-200';
  return <span className={`inline-flex w-fit rounded-full px-3 py-1.5 text-xs font-bold ${cls}`}>{children}</span>;
}
...
function DetailBox({ label, value, white = false }: { label: string; value: string; white?: boolean }) {
  return (
    <div className={`rounded-2xl p-4 ring-1 ring-slate-100 ${white ? 'bg-white' : 'bg-slate-50'}`}>
      <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">{label}</p>
      <p className="mt-1.5 text-sm font-bold text-slate-900">{value}</p>
    </div>
  );
...
function ModalHead({ title, subtitle, onClose }: { title: string; subtitle: string; onClose: () => void }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-6">
      ...
      <button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200">
        <X size={15} />
      </button>
    </div>
  );
...
function PersonaModal({ title, form, setForm, error, loading, onClose, onSave, alumno, apoderado, aviso, parentesco, onParentescoChange }: any) {
  const set = (key: keyof PersonaForm, value: string) => setForm({ ...form, [key]: value });
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 px-4 backdrop-blur-sm animate-modal-overlay-in">
      <div className="w-full max-w-3xl overflow-hidden rounded-[28px] bg-white shadow-2xl ring-1 ring-slate-200 animate-modal-panel-in">
        ...
        <div className="max-h-[70vh] overflow-y-auto p-6">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            {apoderado && onParentescoChange && (
              <label>
                ...
                <select value={parentesco || 'Madre'} onChange={(e) => onParentescoChange(e.target.value)} className={selectClass}>
                  {parentescos.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
              </label>
            )}
          ...
          {aviso && <div className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700 ring-1 ring-amber-200">{aviso}</div>}
          {error && <div className="mt-4 rounded-2xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 ring-1 ring-rose-200">{error}</div>}
        </div>
        <div className="flex flex-col-reverse gap-3 border-t border-slate-100 p-6 sm:flex-row sm:justify-end">
          <button type="button" onClick={onClose} className="h-10 rounded-xl border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Cancelar</button>
          <button type="button" onClick={onSave} disabled={loading} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-accent-500 px-5 text-sm font-bold text-white transition hover:bg-accent-600 disabled:opacity-50">
