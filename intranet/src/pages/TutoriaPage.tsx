  ...
  return (
    <div className="space-y-5 erp-page-enter">
      ...
      <section className="tutoria-main-grid grid items-start gap-5 xl:grid-cols-[380px_minmax(0,1fr)] erp-section-enter">
        ...
        <main className="tutoria-panel-detalle self-start min-h-[720px] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-md shadow-slate-200/80 erp-detail-enter">
          {loadingDetalle ? <div className="flex min-h-[520px] items-center justify-center bg-slate-50/70"><div className="flex items-center gap-2 rounded-2xl bg-slate-50 px-5 py-4 text-sm font-bold text-slate-500"><Loader2 size={16} className="animate-spin" /> Cargando resumen...</div></div> : !detalle ? <div className="flex min-h-[520px] items-center justify-center bg-slate-50/70 p-8 text-center"><div><BookOpenCheck className="mx-auto text-slate-300" size={42} /><h2 className="mt-4 text-lg font-black text-slate-800">Selecciona un alumno</h2><p className="mt-2 max-w-md text-sm text-slate-500">Aquí aparecerán sus notas consolidadas, detalle por evaluación, conducta, participación familiar y comentario final.</p></div></div> : (
            <div key={`${detalle.alumno.id_matricula}-${periodoId}`} className="space-y-5 bg-slate-50/80 p-5 erp-detail-enter">
              <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/70"><div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Alumno seleccionado</p><h2 className="mt-1 text-2xl font-black text-slate-950">{detalle.alumno.nombre}</h2><p className="mt-1 text-sm font-semibold text-slate-500">{detalle.alumno.codigo} · {detalle.alumno.salon}</p>
              ...
              {detalle.asistencia && (
                <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm shadow-slate-200/60">
                  ...
                  <div className="space-y-4 bg-slate-50/70 p-4">
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
                      {[
                        ...
                        <div key={String(label)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm shadow-slate-200/50">
