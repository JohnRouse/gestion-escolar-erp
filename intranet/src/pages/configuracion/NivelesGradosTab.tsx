...
export default function NivelesGradosTab() {
  ...
  const totalGradosCargados = useMemo(
    () =>
      (Object.values(grados) as Grado[][]).reduce(
  ...
  const nombreNivelNormalizado = normalizeLevelName(nombre);
  const nivelDuplicado =
    modal?.type === "nivel" &&
    nombreNivelNormalizado.length > 0 &&
    niveles.some(
      (nivel) =>
        (modal.mode !== "edit" || nivel.id_nivel !== modal.nivel.id_nivel) &&
        normalizeLevelName(nivel.nombre_nivel) === nombreNivelNormalizado,
    );
  ...
  const fetchGrados = async (nivelId: number, force = false) => {
    ...
    try {
      const res = await axios.get(
        `/api/academicos/grados${scopedQuery}&nivel_id=${nivelId}`,
        authHeader,
      );
      ...
      setMensaje({
        type: "error",
        text: "No se pudieron cargar los grados del nivel.",
      });
    }
  };
  ...
  const handleSave = async () => {
    ...
    const cleanName = nombre.replace(/\s+/g, " ").trim();
    ...
    if (modal.type === "nivel" && nivelDuplicado) {
      setMensaje({
        type: "error",
        text: `El nivel "${cleanName}" ya está configurado en esta institución.`,
      });
      return;
    }
    ...
    try {
      ...
      let toastMessage = `Cambios aplicados para ${scopeLabel}.`;

      if (modal.type === "nivel") {
        if (modal.mode === "edit") {
          await axios.put(
            `/api/academicos/niveles/${modal.nivel.id_nivel}${scopedQuery}`,
            {
          ...
          successText = `Nivel "${cleanName}" actualizado correctamente.`;
          toastTitle = "Nivel actualizado";
          toastMessage = successText;
        } else {
          const response = await axios.post<LevelCreateResult>(
            `/api/academicos/niveles${scopedQuery}`,
            {
              nombre_nivel: cleanName,
          ...
          const levelName = response.data.nivel.nombre_nivel;

          if (response.data.yaConfigurado) {
            successText = `El nivel "${levelName}" ya estaba configurado en esta institución.`;
            toastTitle = "Nivel ya configurado";
          } else if (response.data.creado) {
            successText = `Nivel "${levelName}" agregado correctamente.`;
            toastTitle = "Nivel agregado";
          } else if (response.data.vinculado) {
            successText = `Nivel "${levelName}" vinculado correctamente a la institución.`;
            toastTitle = "Nivel vinculado";
          }
        ...
        successText = `Grado "${cleanName}" actualizado correctamente.`;
        toastTitle = "Grado actualizado";
        toastMessage = successText;
      }
      ...
      setMensaje({
        type: "error",
        text: getApiErrorMessage(error, "No se pudo guardar."),
      });
    } finally {
      setSaving(false);
    }
  ...
  const ejecutarEliminarNivel = async (nivel: Nivel) => {
    try {
      await axios.delete(
        `/api/academicos/niveles/${nivel.id_nivel}${scopedQuery}`,
        authHeader,
      );
      setNiveles((prev) =>
        prev.filter((item) => item.id_nivel !== nivel.id_nivel),
      );
      ...
      setMensaje({
        type: "error",
        text: err.response?.data?.message || "No se pudo eliminar el nivel.",
      });
    }
  };
  ...
  const ejecutarEliminarGrado = async (nivel: Nivel, grado: Grado) => {
    try {
      await axios.delete(
        `/api/academicos/grados/${grado.id_grado}${scopedQuery}`,
        authHeader,
      );
      ...
      setMensaje({
        type: "error",
        text: err.response?.data?.message || "No se pudo eliminar el grado.",
      });
    }
  };
  ...
  const confirmarEliminacion = async () => {
    ...
    try {
      if (item.type === "grado") {
        await ejecutarEliminarGrado(item.nivel, item.grado);
      } else {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Ordena Inicial, Primaria y Secundaria con sus grados respectivos.
            Contexto: {nombreColegioGestion}.
          </p>
        </div>
        <button
          type="button"
      ...
      {mostrarSelectorInstitucion && (
        <section className={`${panelClass} p-4`}>
          ...
          <p className="mt-2 text-xs font-semibold text-gray-500">
            En vista consolidada, primero elige la institución para evitar
            mezclar grados entre colegios.
          </p>
        </section>
      )}
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className={`${panelClass} p-4`}>
          ...
          <p className="mt-3 truncate text-lg font-semibold text-gray-950">
            {nivelExpandido?.nombre_nivel || "Ninguno"}
          </p>
      ...
      {niveles.length === 0 ? (
        ...
        >
          ...
          <h4 className="text-base font-semibold text-gray-900">
            Aún no hay niveles educativos
          </h4>
          <p className="mt-1 max-w-md text-sm text-gray-500">
            Crea tu primer nivel para empezar a organizar grados, secciones y
            cursos.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {niveles.map((nivel, index) => {
            ...
            return (
              <article
                key={nivel.id_nivel}
                className={`${panelClass} overflow-hidden transition-all hover:-translate-y-0.5`}
              >
                <div
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-gray-500">
                        {gradosDelNivel.length > 0
                          ? `${gradosDelNivel.length} grados cargados`
                          : "Haz clic para ver sus grados"}
                      </p>
                    </div>
                  </div>
      ...
      <CenteredFormModal
        open={Boolean(modal)}
        eyebrow={
          modal?.type === "grado" ? "Grado académico" : "Nivel educativo"
        }
        title={modal?.mode === "edit" ? "Editar registro" : "Crear registro"}
        description={
          modal?.type === "grado"
        ...
        saving={saving}
        submitLabel="Guardar"
        submitDisabled={!nombre.trim() || nivelDuplicado}
      ...
      >
        <label>
          ...
          <input
            ...
            placeholder={
              modal?.type === "nivel" ? "Ej. Primaria" : "Ej. Quinto Grado"
            }
            autoFocus
          />

          {modal?.type === "nivel" && nivelDuplicado && (
            <span className="mt-1.5 block text-xs font-semibold text-red-600">
              El nivel “{nombre.trim()}” ya está configurado en esta
              institución.
            </span>
          )}
        </label>
      ...
      {batchNivel && (
        <GradeBatchModal
          ...
          open
          nivel={batchNivel}
          existingGrades={grados[batchNivel.id_nivel] || []}
          saving={saving}
          error={mensaje?.type === "error" ? mensaje.text : null}
