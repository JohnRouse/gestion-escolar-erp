...
export default function PlantillasTab() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [plantRes, tiposRes, nivRes, curRes] = await Promise.all([
      ...
      setMensaje({
        tipo: 'error',
        texto: err.response?.data?.message || 'No se pudieron cargar las plantillas.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const getTipoNombre = (idTipo: number) =>
    tipos.find((tipo) => tipo.id_tipo_eval === idTipo)?.nombre_tipo || 'Evaluación';
    ...
    return plantillas.filter((plantilla) => {
      ...
      const texto = normalizeText(
        [
          ...
          ...(plantilla.detalles || []).map((detalle) => detalle.descripcion || detalle.tipo?.nombre_tipo || ''),
        ].join(' ')
      );

      return coincideAlcance && (!query || texto.includes(query));
    });
  }, [busqueda, filtroAlcance, plantillas]);
  ...
  const openEdit = (plantilla: Plantilla) => {
    ...
    setIdNivel(plantilla.id_nivel);
    setIdCurso(plantilla.id_curso);
    setDetalles(
  ...
  const nextPracticeNumber = () => {
    const practiceCount = detalles.filter((detalle) =>
      normalizeText(getTipoNombre(detalle.id_tipo_eval)).includes('practica')
    ).length;

    return practiceCount + 1;
  };

  const buildSuggestedDescription = (tipoId: number) => {
    const tipoNombre = getTipoNombre(tipoId);
    const tipoNormalizado = normalizeText(tipoNombre);

    if (tipoNormalizado.includes('practica')) return `Práctica ${nextPracticeNumber()}`;
  ...
  const addDetalle = (tipoId?: number) => {
    ...
    setDetalles((prev) => [
      ...prev,
      {
        ...
        descripcion: buildSuggestedDescription(selectedTipoId),
        orden: prev.length + 1,
      },
    ]);
  };
  ...
  const aplicarEstructuraBase = () => {
    const findTipo = (name: string) =>
      tipos.find((tipo) => normalizeText(tipo.nombre_tipo).includes(normalizeText(name)))?.id_tipo_eval ||
      tipos[0]?.id_tipo_eval ||
      0;

    const base = [
      { tipo: findTipo('participacion'), descripcion: 'Participación' },
      { tipo: findTipo('cuaderno'), descripcion: 'Cuaderno' },
      { tipo: findTipo('practica'), descripcion: 'Práctica 1' },
      { tipo: findTipo('practica'), descripcion: 'Práctica 2' },
      { tipo: findTipo('practica'), descripcion: 'Práctica 3' },
      { tipo: findTipo('examen'), descripcion: 'Examen' },
    ];
  ...
  const updateDetalle = (index: number, field: keyof DetallePlantilla, value: string | number) => {
    setDetalles((prev) =>
      prev.map((detalle, currentIndex) => {
        ...
        const nextDetalle = { ...detalle, [field]: value };

        if (field === 'id_tipo_eval') {
          nextDetalle.descripcion = buildSuggestedDescription(Number(value));
        }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-[1.45rem] border border-gray-200/70 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Plantillas</p>
          <div className="mt-3 flex items-end justify-between">
            <p className="text-3xl font-semibold tracking-[-0.04em] text-gray-950">{stats.total}</p>
        ...
        <div className="rounded-[1.45rem] border border-gray-200/70 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Globales</p>
          <div className="mt-3 flex items-end justify-between">
            <p className="text-3xl font-semibold tracking-[-0.04em] text-gray-950">{stats.globales}</p>
        ...
        <div className="rounded-[1.45rem] border border-gray-200/70 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Por nivel</p>
          <div className="mt-3 flex items-end justify-between">
            <p className="text-3xl font-semibold tracking-[-0.04em] text-gray-950">{stats.porNivel}</p>
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gray-50 text-gray-500">
        ...
        <div className="rounded-[1.45rem] border border-gray-200/70 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Por curso</p>
          <div className="mt-3 flex items-end justify-between">
            <p className="text-3xl font-semibold tracking-[-0.04em] text-gray-950">{stats.porCurso}</p>
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gray-50 text-gray-500">
        ...
        <div className="rounded-[1.45rem] border border-gray-200/70 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-400">Evaluaciones</p>
          <div className="mt-3 flex items-end justify-between">
            <p className="text-3xl font-semibold tracking-[-0.04em] text-gray-950">{stats.totalEvaluaciones}</p>
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gray-50 text-gray-500">
              <Layers3 size={19} />
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Define desde el inicio del año qué columnas aparecerán en la grilla de notas.
                  </p>
                </div>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <button
              type="button"
              onClick={openCreate}
              className="mt-5 inline-flex h-10 items-center gap-2 rounded-2xl bg-accent-500 px-4 text-sm font-semibold text-white shadow-sm hover:bg-accent-600"
            >
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-[0_30px_100px_-35px_rgba(15,23,42,0.55)]">
            <div className="flex items-start justify-between gap-4 border-b border-gray-100 p-5 sm:p-6">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-gray-500">
                  Configura la estructura que luego se usará para generar las columnas del registro de notas.
                </p>
              </div>
              ...
              <div className="space-y-5 p-5 sm:p-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej. Evaluaciones General Primaria"
                    className="h-12 w-full rounded-2xl border border-gray-200 bg-white px-4 text-sm font-medium text-gray-800 outline-none transition placeholder:text-gray-300 focus:border-accent-200 focus:ring-4 focus:ring-accent-50"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    {[
                      { key: 'global', label: 'Todo el colegio', icon: Building2, help: 'Todos los niveles y cursos' },
                      { key: 'nivel', label: 'Por nivel', icon: GraduationCap, help: 'Inicial, Primaria o Secundaria' },
                      { key: 'curso', label: 'Por curso', icon: BookOpenCheck, help: 'Solo un curso específico' },
                    ].map((item) => {
                      const Icon = item.icon;
                      const active = alcance === item.key;

                      return (
                        <button
                          key={item.key}
                          type="button"
                          onClick={() => handleChangeAlcance(item.key as Alcance)}
                ...
                <div className="sticky top-0 space-y-4">
                    ...
                    <div className="mt-4 space-y-3 text-sm">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="rounded-full bg-accent-50 px-3 py-1 text-xs font-semibold text-accent-600">
                          {alcance === 'global' ? 'Global' : alcance === 'nivel' ? 'Nivel' : 'Curso'}
                        </span>
                      </div>
                      <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="rounded-[1.5rem] border border-gray-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-4 overflow-hidden rounded-2xl border border-gray-200">
                      <div className="grid grid-cols-[1.2fr_repeat(3,minmax(72px,1fr))] bg-gray-50 text-center text-[11px] font-bold uppercase tracking-[0.12em] text-gray-400">
                        <div className="border-r border-gray-200 px-3 py-2 text-left">Alumno</div>
                        {detalles.slice(0, 3).map((detalle, index) => (
                          <div key={`${detalle.descripcion}-${index}`} className="border-r border-gray-200 px-3 py-2 last:border-r-0">
                      ...
                      <div className="grid grid-cols-[1.2fr_repeat(3,minmax(72px,1fr))] text-center text-xs font-semibold text-gray-700">
                        <div className="border-r border-t border-gray-200 px-3 py-3 text-left">Estudiante</div>
                        <div className="border-r border-t border-gray-200 px-3 py-3">00</div>
                        <div className="border-r border-t border-gray-200 px-3 py-3">00</div>
