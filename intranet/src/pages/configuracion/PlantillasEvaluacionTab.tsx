...
const gruposEvaluacion = ['Trabajo en clase', 'Prácticas', 'Examen'] as const;
...
interface PlantillaDetalle { id_detalle?: number; id_tipo_eval: number; descripcion: string; grupo_evaluacion?: string; orden: number; tipo?: TipoEval; }
...
const detalleVacio = (orden: number, tipoDefault?: number): PlantillaDetalle => ({
  ...
  descripcion: '',
  grupo_evaluacion: 'Trabajo en clase',
  orden,
});
...
export default function PlantillasEvaluacionTab() {
  ...
  const openEdit = (plantilla: Plantilla) => {
    ...
    setDetalles(
      plantilla.detalles?.length
        ? plantilla.detalles.slice().sort((a, b) => a.orden - b.orden).map((d, i) => ({
            id_tipo_eval: d.id_tipo_eval || d.tipo?.id_tipo_eval || tipoDefault || 0,
            descripcion: d.descripcion || '',
            grupo_evaluacion: d.grupo_evaluacion || 'Trabajo en clase',
            orden: d.orden || i + 1,
          }))
  ...
  const handleSave = async () => {
    ...
    const cleanDetalles = detalles
      .filter((d) => d.id_tipo_eval && d.descripcion.trim())
      .map((detalle, index) => ({
        id_tipo_eval: Number(detalle.id_tipo_eval),
        descripcion: detalle.descripcion.trim(),
        grupo_evaluacion: detalle.grupo_evaluacion || 'Trabajo en clase',
        orden: index + 1,
      }));
