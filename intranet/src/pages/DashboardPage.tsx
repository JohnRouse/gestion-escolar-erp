  ...
  return (
    <div className="carbon-dashboard w-full space-y-6">
      ...
      <div className={`space-y-8 transition-all duration-500 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        {loading ? (
          <section className="carbon-dashboard-hero grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[...Array(4)].map((_, i) => (<div key={i} className="min-h-[120px] rounded border border-slate-200 bg-white p-5 shadow-none animate-pulse"><div className="h-3 w-20 rounded bg-neutral-100 mb-4" /><div className="h-7 w-16 rounded bg-neutral-100 mb-3" /><div className="h-3 w-28 rounded bg-neutral-100" /></div>))}
          </section>
        ) : (
          <>
            {institucional && (
              ...
              <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  {institucional.colegios.map((colegio: any) => (
                    <div key={colegio.id_colegio} className="school-summary-card rounded-sm border border-slate-200 bg-slate-50 p-4 transition hover:bg-white">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className="shrink-0 rounded-sm bg-white px-2.5 py-1 text-[11px] font-semibold text-neutral-500 ring-1 ring-slate-200">{colegio.codigo}</span>
                      </div>
                      <div className="mt-5 grid grid-cols-3 gap-3">
                        <div className="rounded-sm border border-slate-200 bg-white p-3"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Alumnos</p><p className="mt-1 text-lg font-semibold text-neutral-900 tabular-nums">{formatNumber(colegio.kpis?.matriculados)}</p></div>
                        <div className="rounded-sm border border-slate-200 bg-white p-3"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Docentes</p><p className="mt-1 text-lg font-semibold text-neutral-900 tabular-nums">{formatNumber(colegio.kpis?.docentes)}</p></div>
                        <div className="rounded-sm border border-slate-200 bg-white p-3"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Pagos</p><p className="mt-1 text-lg font-semibold text-neutral-900 tabular-nums">{formatNumber(colegio.kpis?.pagosPendientes)}</p></div>
                      </div>
                      <div className="mt-5">
                ...
                {!institucional.avanceCargaDocente.unidadActual ? (
                  ...
                  <div className="space-y-3">
                    {institucional.avanceCargaDocente.docentes.slice(0, 8).map((item: any) => (
                      <div key={item.id_docente} className="rounded-sm border border-slate-200 bg-slate-50 p-4 transition hover:bg-white">
            ...
            {docente && (
              <section className="space-y-6">
                  ...
                  {!docente.unidadActual ? (<EmptyPanel title="No hay unidad aperturada" description="Cuando Dirección o Administración abra una unidad, aquí aparecerá el avance de carga de notas." />) : docente.avancePorCurso?.length ? (
                    <div className="space-y-3">
                      {docente.avancePorCurso.map((item: any) => (
                        <div key={item.id_asignacion} className="rounded-sm border border-slate-200 bg-slate-50 p-4 transition hover:bg-white">
                ...
                <div className="grid gap-6 xl:grid-cols-3">
                  <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60 xl:col-span-2">
                    ...
                    {docente.seccionesResumen?.length ? (
                      <div className="space-y-3">
                        {docente.seccionesResumen.map((seccion: any) => (
                          <div key={seccion.id_seccion} className="rounded-sm border border-slate-200 bg-slate-50 p-4 flex flex-col gap-2 transition hover:bg-white sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <span className="shrink-0 inline-flex w-fit rounded-sm bg-white px-2.5 py-1 text-[11px] font-semibold text-neutral-500 ring-1 ring-slate-200">{seccion.totalAlumnos} alumnos</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
                    ...
                    <div className="space-y-3">
                      ...
                      <button type="button" onClick={() => navigate('/asistencia')} className="flex w-full items-center justify-between rounded-sm border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700 transition hover:border-slate-900">
              ...
              <section className="space-y-6">
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  {tutoria.secciones?.map((seccion: any) => (
                    <div key={seccion.id_seccion} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                      ...
                      <div className="mt-5 grid grid-cols-2 gap-3">
                        <div className="rounded bg-slate-50 p-4 ring-1 ring-slate-200"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Alumnos</p><p className="mt-1.5 text-xl font-semibold text-neutral-900 tabular-nums">{formatNumber(seccion.alumnos)}</p></div>
                        <div className="rounded bg-slate-50 p-4 ring-1 ring-slate-200"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Promedio</p><p className="mt-1.5 text-xl font-semibold text-neutral-900 tabular-nums">{formatNota(seccion.promedioGeneral)}</p></div>
            ...
            {false && institucional && (
              <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
                ...
                <div className="school-summary-card rounded-sm border border-slate-200 bg-slate-50 p-4 transition hover:bg-white">
