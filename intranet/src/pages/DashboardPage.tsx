...
function KpiCard({ label, value, helper, icon: Icon, tone = 'neutral', onClick }: { label: string; value: string | number; helper: string; icon: any; tone?: 'neutral' | 'blue' | 'emerald' | 'amber' | 'rose' | 'violet'; onClick?: () => void; }) {
  ...
  const baseClass = "min-h-[120px] rounded-2xl border border-neutral-200/60 bg-white p-5 text-left shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-all duration-150";

  if (onClick) {
    return (
      <button type="button" onClick={onClick} className={`${baseClass} group cursor-pointer hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)] hover:-translate-y-0.5`}>
        {content}
      </button>
    );
  }
...
function EmptyPanel({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex min-h-[200px] flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 bg-neutral-50/50 px-6 py-8 text-center">
      <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-neutral-100 text-neutral-300"><Sparkles size={20} /></div>
...
function ProgressBar({ value }: { value: number }) {
  return (
    <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-neutral-200/60">
      <div className="h-full rounded-full bg-[#CCF32F] transition-all duration-700" style={{ width: `${Math.min(100, Math.max(0, value || 0))}%` }} />
    </div>
  );
}
...
function EstadoCargaBadge({ estado }: { estado: string }) {
  ...
  return (
    <span className={cx('inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1', estadoStyles[estado] || estadoStyles.pendiente)}>
  ...
  const SectionHeader = ({ icon: Icon, title, subtitle, action }: { icon: any; title: string; subtitle: string; action?: React.ReactNode }) => (
    <div className="flex items-center justify-between gap-3 mb-5">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#CCF32F]/10 text-[#CCF32F]"><Icon size={18} strokeWidth={2} /></div>
      ...
      <div className={`transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
        <PageHeader
          ...
          actions={rol === 'Admin' || rol === 'Director' ? (
            <button type="button" onClick={() => navigate('/reportes')} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-neutral-900 px-4 text-sm font-medium text-white transition-all duration-150 hover:bg-neutral-800 hover:scale-[1.02] active:scale-[0.98]">
      ...
      <div className={`space-y-8 transition-all duration-500 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        {loading ? (
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[...Array(4)].map((_, i) => (<div key={i} className="min-h-[120px] rounded-2xl border border-neutral-200/60 bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] animate-pulse"><div className="h-3 w-20 rounded bg-neutral-100 mb-4" /><div className="h-7 w-16 rounded bg-neutral-100 mb-3" /><div className="h-3 w-28 rounded bg-neutral-100" /></div>))}
          </section>
        ) : (
          <>
            {institucional && (
              ...
              <section className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  {institucional.colegios.map((colegio: any) => (
                    <div key={colegio.id_colegio} className="rounded-2xl bg-neutral-50 p-5 ring-1 ring-neutral-200/60">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex items-center gap-3">
                          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-white shadow-sm" style={{ backgroundColor: colegio.color_principal || '#4f46e5' }}><School size={19} /></span>
                        ...
                        <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-neutral-500 ring-1 ring-neutral-200/60">{colegio.codigo}</span>
                      </div>
                      <div className="mt-5 grid grid-cols-3 gap-3">
                        <div className="rounded-xl bg-white p-3 ring-1 ring-neutral-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Alumnos</p><p className="mt-1 text-lg font-semibold text-neutral-900 tabular-nums">{formatNumber(colegio.kpis?.matriculados)}</p></div>
                        <div className="rounded-xl bg-white p-3 ring-1 ring-neutral-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Docentes</p><p className="mt-1 text-lg font-semibold text-neutral-900 tabular-nums">{formatNumber(colegio.kpis?.docentes)}</p></div>
                        <div className="rounded-xl bg-white p-3 ring-1 ring-neutral-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Pagos</p><p className="mt-1 text-lg font-semibold text-neutral-900 tabular-nums">{formatNumber(colegio.kpis?.pagosPendientes)}</p></div>
                      </div>
                      <div className="mt-5">
              ...
              <section className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                <SectionHeader icon={Table2} title="Avance de carga por docente" subtitle="Estado de notas registradas en la unidad aperturada." action={
                  institucional.avanceCargaDocente.unidadActual ? (
                    <span className="inline-flex w-fit rounded-full bg-emerald-50/50 px-2.5 py-1 text-[11px] font-semibold text-emerald-600 ring-1 ring-emerald-200/60">Bimestre {institucional.avanceCargaDocente.unidadActual.bimestre} · Unidad {institucional.avanceCargaDocente.unidadActual.numero}</span>
                  ) : (<span className="inline-flex w-fit rounded-full bg-amber-50/50 px-2.5 py-1 text-[11px] font-semibold text-amber-600 ring-1 ring-amber-200/60">Sin unidad abierta</span>)
                } />
                {!institucional.avanceCargaDocente.unidadActual ? (
                  ...
                  <div className="space-y-3">
                    {institucional.avanceCargaDocente.docentes.slice(0, 8).map((item: any) => (
                      <div key={item.id_docente} className="rounded-xl bg-neutral-50 p-4 ring-1 ring-neutral-200/60 transition-all duration-150 hover:ring-neutral-300/60">
              ...
              <section className="space-y-6">
                ...
                <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                  <SectionHeader icon={Table2} title="Avance de carga por curso" subtitle="Progreso de notas registradas en la unidad aperturada." action={
                    docente.unidadActual ? (<span className="inline-flex w-fit rounded-full bg-emerald-50/50 px-2.5 py-1 text-[11px] font-semibold text-emerald-600 ring-1 ring-emerald-200/60">Unidad {docente.unidadActual.numero} abierta</span>) : (<span className="inline-flex w-fit rounded-full bg-amber-50/50 px-2.5 py-1 text-[11px] font-semibold text-amber-600 ring-1 ring-amber-200/60">Sin unidad abierta</span>)
                  } />
                  {!docente.unidadActual ? (<EmptyPanel title="No hay unidad aperturada" description="Cuando Dirección o Administración abra una unidad, aquí aparecerá el avance de carga de notas." />) : docente.avancePorCurso?.length ? (
                    <div className="space-y-3">
                      {docente.avancePorCurso.map((item: any) => (
                        <div key={item.id_asignacion} className="rounded-xl bg-neutral-50 p-4 ring-1 ring-neutral-200/60 transition-all duration-150 hover:ring-neutral-300/60">
                ...
                <div className="grid gap-6 xl:grid-cols-3">
                  <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] xl:col-span-2">
                    ...
                    {docente.seccionesResumen?.length ? (
                      <div className="space-y-3">
                        {docente.seccionesResumen.map((seccion: any) => (
                          <div key={seccion.id_seccion} className="rounded-xl bg-neutral-50 p-4 ring-1 ring-neutral-200/60 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <span className="shrink-0 inline-flex w-fit rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-neutral-500 ring-1 ring-neutral-200/60">{seccion.totalAlumnos} alumnos</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                    ...
                    <div className="space-y-3">
                      <button type="button" onClick={() => navigate('/notas')} className="flex w-full items-center justify-between rounded-xl bg-neutral-900 px-4 py-3 text-sm font-medium text-white transition-all duration-150 hover:bg-neutral-800 hover:scale-[1.01] active:scale-[0.99]">
                        Registro de notas <ArrowUpRight size={16} />
                      </button>
                      <button type="button" onClick={() => navigate('/asistencia')} className="flex w-full items-center justify-between rounded-xl bg-neutral-50 px-4 py-3 text-sm font-medium text-neutral-700 ring-1 ring-neutral-200/60 transition-all duration-150 hover:bg-white hover:ring-neutral-300/60">
              ...
              <section className="space-y-6">
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  {tutoria.secciones?.map((seccion: any) => (
                    <div key={seccion.id_seccion} className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="inline-flex items-center gap-1.5 rounded-full bg-violet-50/50 px-2.5 py-1 text-[11px] font-semibold text-violet-600 ring-1 ring-violet-200/60"><MessageSquareText size={12} /> Tutoría activa</div>
                        ...
                        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50/50 text-violet-500"><Users size={18} /></div>
                      </div>
                      <div className="mt-5 grid grid-cols-2 gap-3">
                        <div className="rounded-xl bg-neutral-50 p-4 ring-1 ring-neutral-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Alumnos</p><p className="mt-1.5 text-xl font-semibold text-neutral-900 tabular-nums">{formatNumber(seccion.alumnos)}</p></div>
                        <div className="rounded-xl bg-neutral-50 p-4 ring-1 ring-neutral-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Promedio</p><p className="mt-1.5 text-xl font-semibold text-neutral-900 tabular-nums">{formatNota(seccion.promedioGeneral)}</p></div>
                        <div className="rounded-xl bg-red-50/50 p-4 ring-1 ring-red-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-red-400">En riesgo</p><p className="mt-1.5 text-xl font-semibold text-red-600 tabular-nums">{formatNumber(seccion.alumnosRiesgo)}</p></div>
                        <div className="rounded-xl bg-amber-50/50 p-4 ring-1 ring-amber-200/60"><p className="text-[10px] font-semibold uppercase tracking-widest text-amber-500">Coment. pend.</p><p className="mt-1.5 text-xl font-semibold text-amber-600 tabular-nums">{formatNumber(seccion.comentariosPendientes)}</p></div>
                      </div>
                      <button type="button" disabled className="mt-5 flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-neutral-100 px-4 py-3 text-sm font-medium text-neutral-400">
                ...
                {institucional.eventosProximos.length === 0 ? (
                  ...
                  <div className="divide-y divide-neutral-100">
                    {institucional.eventosProximos.map((evento: any) => {
                      ...
                      return (
                        <div key={evento.id_evento} className="flex items-start gap-4 py-3 first:pt-0 last:pb-0 transition-colors duration-150 hover:bg-neutral-50/50 -mx-2 px-2 rounded-xl">
                          <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl bg-[#CCF32F] text-black shadow-sm">
                          ...
                          <div className="min-w-0 flex-1 pt-0.5">
                            ...
                            <div className="mt-1.5 flex flex-wrap items-center gap-2">
                              ...
                              {evento.tipo && (<span className="rounded-full bg-neutral-100 px-2 py-0.5 text-[10px] font-semibold text-neutral-500">{evento.tipo}</span>)}
                            </div>
                          </div>
                        </div>
                      );
            ...
            {institucional && (
              <section className="rounded-2xl border border-neutral-200/60 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                <SectionHeader icon={Wallet} title="Estado de pagos" subtitle="Seguimiento operativo de tesorería." action={
                  <button type="button" onClick={() => navigate('/tesoreria')} className="inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-neutral-900 px-3.5 text-xs font-medium text-white transition-all duration-150 hover:bg-neutral-800 hover:scale-[1.02] active:scale-[0.98]">
                ...
                <div className="rounded-xl bg-neutral-50 p-5 ring-1 ring-neutral-200/60">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <span className="shrink-0 inline-flex w-fit rounded-full bg-red-50/50 px-2.5 py-1 text-[11px] font-semibold text-red-500 ring-1 ring-red-200/60">Pendiente</span>
                  </div>
                </div>
              </section>
            )}
