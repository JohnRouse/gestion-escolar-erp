...
export default function DashboardPage() {
  ...
  const fetchHijos = async (token: string) => {
    try {
      ...
      setHijos(hijosData);
      if (!selectedChild && hijosData.length > 0) setSelectedChild(hijosData[0]);
      if (selectedChild) fetchDashboardData(token, selectedChild.id_estudiante);
  ...
  const fetchDashboardData = async (token: string, alumnoId: number) => {
    ...
    try {
      const [asistRes, notasRes, pagosRes, circRes, actRes] = await Promise.allSettled([
        ...
        axios.get(`/api/actividad?alumno_id=${alumnoId}&limite=3`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  return (
    <main className="min-h-screen bg-surface-alt pb-24">
      <DashboardHeader />
      <div key={selectedChild?.id_estudiante} className={`-mt-4 px-5 pb-6 relative z-20 transition-opacity duration-300 ${fadeIn ? "opacity-100" : "opacity-0"}`}>
        {loading ? (
          <div className="grid grid-cols-2 gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="m-card p-4 space-y-3"><div className="skel h-3 w-16" /><div className="skel h-8 w-20" /><div className="skel h-2 w-full" /></div>
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={() => router.push("/dashboard/asistencia")} className="press m-card p-4 text-left">
              <div className="flex items-center justify-between">
                ...
                <span className="w-7 h-7 rounded-full bg-success-soft flex items-center justify-center"><span className="material-symbols-rounded text-success text-lg">check</span></span>
              </div>
              ...
              <div className="mt-3 h-1.5 bg-border rounded-full overflow-hidden"><div className="h-full bg-success transition-all duration-700" style={{ width: `${dashboardData?.asistencia ?? 0}%` }} /></div>
            </button>
            {/* Promedio */}
            <button onClick={() => router.push("/dashboard/calificaciones")} className="press m-card p-4 text-left">
              <div className="flex items-center justify-between">
                ...
                <span className="w-7 h-7 rounded-full bg-accent-soft flex items-center justify-center"><span className="material-symbols-rounded text-accent text-lg">trending_up</span></span>
              </div>
              ...
              <span className={`inline-flex mt-3 px-2 py-0.5 rounded-full text-[11px] font-bold ${(dashboardData?.promedio ?? 0) >= 11 ? "bg-success-soft text-success" : "bg-danger-soft text-danger"}`}>{(dashboardData?.promedio ?? 0) >= 11 ? "Aprobado" : "En riesgo"}</span>
            </button>
            {/* Pagos */}
            <button onClick={() => router.push("/dashboard/pagos")} className="press m-card p-4 text-left">
              <div className="flex items-center justify-between">
                ...
                <span className="w-7 h-7 rounded-full bg-danger-soft flex items-center justify-center"><span className="material-symbols-rounded text-danger text-lg">credit_card</span></span>
              </div>
              ...
              <span className={`inline-flex mt-3 px-2 py-0.5 rounded-full text-[11px] font-bold ${dashboardData?.estadoPagos === "Al día" ? "bg-success-soft text-success" : "bg-warning-soft text-warning"}`}>{dashboardData?.estadoPagos === "Al día" ? "Al día" : "Por pagar"}</span>
            </button>
            {/* Último aviso */}
            <button onClick={() => router.push("/dashboard/circulares")} className="press m-card p-4 text-left">
              <div className="flex items-center justify-between">
                ...
                <span className="w-7 h-7 rounded-full bg-info-soft flex items-center justify-center"><span className="material-symbols-rounded text-info text-lg">campaign</span></span>
              </div>
              ...
              {dashboardData?.circularReciente && (<p className="text-[11px] text-text-secondary mt-2">{new Date(dashboardData.circularReciente.fecha).toLocaleDateString("es-PE", { day: "2-digit", month: "short" })}</p>)}
            </button>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <button onClick={() => router.push('/dashboard/actividad')} className="text-xs font-bold text-accent">Ver más</button>
        </div>
        {loading ? (
          <div className="mt-3 space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="m-card p-3 flex items-center gap-3"><div className="skel w-10 h-10 rounded-xl" /><div className="flex-1 space-y-2"><div className="skel h-3 w-3/4" /><div className="skel h-2.5 w-1/3" /></div></div>
            ))}
          </div>
        ) : actividad.length > 0 ? (
          <div className="mt-3 space-y-3">
            {actividad.map((evento, idx) => (
              <button key={idx} onClick={() => router.push(evento.url)} className="m-card p-3 flex items-center gap-3 press w-full text-left">
                <span className="w-10 h-10 rounded-xl bg-surface-alt flex items-center justify-center text-lg">{evento.icono}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-text">{evento.mensaje}</p>
                  <p className="text-xs text-text-secondary mt-0.5">{new Date(evento.fecha).toLocaleDateString("es-PE", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</p>
                </div>
          ...
          <p className="text-center text-text-muted text-sm mt-6">No hay actividad reciente.</p>
        )}
      </div>
      <BottomNav />
    </main>
